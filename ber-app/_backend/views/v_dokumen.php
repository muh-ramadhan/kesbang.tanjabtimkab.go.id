		...
		<div class="col-lg-12">
			<div class="panel panel-default">
				<div class="panel-heading">
					<i class="fa fa-th-list fa-fw"></i> Dokumen Terbaru
				</div>
				<div class="panel-body">
						<!--
						<form action="<?php echo base_url(); ?>dokumen/a_deleteall" method="POST">
						-->
						<?php echo form_open_multipart('dokumen/deleteall'); ?>
						<br>
						<center>
							<a class="btn btn-app btn-light btn-xs radius-4">
								<i class="ace-icon fa fa-home bigger-160"></i> Home
							</a>
							<a href="<?php echo base_url(); ?>dokumen/add" class="btn btn-app btn-primary btn-xs radius-4">
								<i class="ace-icon fa fa-plus-circle bigger-160"></i> Tambah
							</a>
							<a href="<?php echo base_url(); ?>dokumen" class="btn btn-app btn-warning  btn-xs  radius-4">
										...
										<tbody>
											...
											foreach($artikel as $key => $row){
												...
												<tr class="odd">
													...
													<td>
														<center>
															<input type="checkbox" name="cek[]" class="case" value="<?php echo $row['id_dokumen']; ?>" id="id<?php echo $no; ?>" rel="ck" title="Pilih"></center>
														</td>
														<td>
															<a class="bold" href="<?php echo base_url(); ?>dokumen/edit/<?php echo $row['id_dokumen']."/".$judul."/";?>"> <?php echo $row['judul']; ?> </a>
														</td>
														<td>
															<?php if ($row['metode_link']=='1') {?>
																<a target="_blank" href="<?php echo base_url(); ?>../dokumens/<?php echo $photopath;?>/<?php echo $row['nama_file'];?>" >Download </a>
															<?php } else {?>
																<a href="<?php echo $row['link_file'];?>" >Download </a>
															<?php } ?>
														</td>
														<td><?php echo $row['nama_katdokumen']; ?></td>
														<td><?php echo $row['tahun']; ?></td>
														<td><?php if ($row['jangkrik']=='Y') {?>
															<a href="<?php echo base_url(); ?>dokumen/nonaktif/<?php echo $row['id_dokumen']."/".$judul."/";?>" class="btn btn-info btn-circle btn-lg">
																<i class="fa fa-check"></i> </a>
															<?php } else {?>
																<a href="<?php echo base_url(); ?>dokumen/aktif/<?php echo $row['id_dokumen']."/".$judul."/";?>" 	class="btn btn-warning btn-circle btn-lg"><i class="fa fa-times"></i> </a>
															<?php } ?>
														</td>
														<td class="center"><?php echo $row['nama_lengkap']; ?></td>
														<td class="center">
															<a href="<?php echo base_url(); ?>dokumen/edit/<?php echo $row['id_dokumen']."/".$judul."/";?>" class="btn btn-block btn-social btn-dropbox">
																<i class="fa fa-pencil"></i> Edit
															</a>
															<a href="<?php echo base_url(); ?>dokumen/delete/<?php echo $row['id_dokumen']."/".$judul."/";?>" class="btn btn-block btn-social btn-pinterest" onclick="return confirm('Apakah Anda benar-benar mau menghapusnya?')">
				...
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header">Tambah Dokumen</h1>
					</div>
				</div>
				<div class="row">
					<div class="col-lg-10">
						<div class="panel panel-default">
							<div class="panel-heading">
								<i class="fa fa-wrench fa-fw"></i> Tambah Dokumen
							</div>
							<div class="panel-body">
								<form method="POST" id="dokumen" name="dokumen" action="<?php echo base_url(); ?>dokumen/a_simpan"  enctype="multipart/form-data" onsubmit="return validasi(this)">
								...
								<div class="form-group">
									...
									<select class="form-control"  name="kategori" id="kategori">
										...
										foreach($dataa->result_array() as $r) {
											?>
											<option  value="<?php echo $r['id_katdokumen'];?>"><?php echo $r['nama_katdokumen'];?></option>
										<?php } ?>
									</select>
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									...
									<script type='text/javascript'>
										$('#drop').change(function() {
											if ($(this).val() == '1') {
												$('#linkmethod').load('<?php echo base_url()?>dokumen/inputfile');
											}
											else if ($(this).val() == '2') {
												$('#linkmethod').load('<?php echo base_url()?>dokumen/custom');
											}
											else {
												$('#linkmethod').load('<?php echo base_url()?>dokumen/custom');
											}
						...
						<div class="row">
							<div class="col-lg-12">
								<h1 class="page-header">Edit Dokumen</h1>
							</div>
						</div>
						<div class="row">
							<div class="col-lg-10">
								<div class="panel panel-default">
									<div class="panel-heading">
										<i class="fa fa-wrench fa-fw"></i>Edit Dokumen
									</div>
									<div class="panel-body">
										<form method="POST" id="dokumen" name="dokumen" action="<?php echo base_url(); ?>dokumen/a_edit"  enctype="multipart/form-data" onsubmit="return validasi(this)">
											<?php
											$edit = $this->M_dataadmin->editdokumen($this->uri->segment(3,0));
											foreach($edit->result_array() as $raw)
											{
												...
												$tanggal=$c.'-'.$b.'-'.$a;
												?>
												<input type="hidden" name="id" value="<?php echo $raw['id_dokumen']; ?>">
												<div class="aksi" style="border-bottom:1px solid #ccc; margin-bottom:20px; ">
													<center>
														<button type="submit" class="btn btn-app btn-primary btn-xs radius-4">
															<i class="ace-icon fa fa-floppy-o bigger-160"></i> Simpan
											...
											<div class="form-group">
												...
												<select class="form-control"  name="kategori" id="kategori">
													<?php
													$dataa = $this->M_dataadmin->pilihkatdokumen();
													if ($raw['id_katdokumen']==0){
														echo "<option value=0 selected>- Pilih -</option>";
													}
													foreach($dataa->result_array() as $raaw) {
														if ($raw['id_katdokumen']==$raaw['id_katdokumen']){ ?>
															<option  value="<?php echo $raaw['id_katdokumen'];?>" selected><?php echo $raaw['nama_katdokumen'];?></option>
														<?php } else {?>
															<option  value="<?php echo $raaw['id_katdokumen'];?>"><?php echo $raaw['nama_katdokumen'];?></option>
														<?php }
													}
													?>
												</select>
											...
											<div id="linkmethod" style="border:1px solid #ec8585;background:#f2f2f2;border-radius:5px;padding:20px;margin-bottom:20px;">
												...
												if ($raw['metode_link']==1){
													if ($raw['nama_file']!='') {
														...
														?><br>
														<a target="_blank" href="<?php echo base_url(); ?>../dokumen/<?php echo $pathdelete?>/<?php echo $raw['nama_file']; ?>"><?php echo $raw['nama_file']; ?> </a>
														<div class="clearfix"></div>
														<a href="<?php echo base_url(); ?>dokumen/hapusfile/<?php echo $this->uri->segment(3,0); ?>" class="btn btn-danger"  onClick=\"return confirm('Apakah Anda benar-benar mau menghapusnya?')\">Hapus File</a>
														<?php
													}
													else {
														?>
													...
													if ($raw['link_file']!='') {
														...
														<a href="<?php echo base_url(); ?>dokumen/hapuslink/<?php echo $this->uri->segment(3,0); ?>"  class="btn btn-danger" onClick=\"return confirm('Apakah Anda benar-benar mau menghapusnya?')\">Hapus Link</a>
